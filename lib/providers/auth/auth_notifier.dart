...
class AuthNotifier extends Notifier<AuthState> {
  ...
  Future<void> _handleSignOut() async {
    try {
      ...
      // Only clear auth-specific keys, preserve remember-me data
      ...
      if (session != null) {
        ...
        try {
          ...
          final employeeResponse = await _supabaseClient
              .from('employees')
              .select(
                  'id, full_name, email, role, department, phone, avatar_url, branch_id, company_id, warehouse_id, is_active, created_at, updated_at, companies(name, business_type)')
  ...
  Future<void> reloadUserFromDatabase() async {
    try {
      ...
      final employeeResponse = await _supabaseClient
          .from('employees')
          .select(
              'id, full_name, email, role, department, phone, avatar_url, branch_id, company_id, warehouse_id, is_active, created_at, updated_at')
  ...
  Future<void> logout() async {
    ...
    try {
      // 1. Clear auth session storage (but preserve remember-me data)
      final prefs = await SharedPreferences.getInstance();
      await prefs.remove(_authStorageKey);
      await prefs.remove(_demoModeKey);

      // 2. Legacy keys cleanup (old format)
      await prefs.remove('saved_email');
      await prefs.remove('saved_password');
      await prefs.setBool('remember_me', false);
      // NOTE: @employee_remember_me, @employee_saved_credentials,
      //       @ceo_remember_me, @ceo_saved_credentials are kept
      //       so users can auto-login on next visit
  ...
  Future<bool> changePassword(
    ...
    try {
      ...
      final currentHash = await client.rpc('hash_password', params: {
