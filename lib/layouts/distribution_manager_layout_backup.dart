  ...
  Widget build(BuildContext context) {
    ...
    final pages = <Widget>[
      _DistributionDashboardPageWithRoleSwitcher(
        onSwitchRole: (role) {
          // Navigate to actual layouts instead of switching view
          switch (role) {
            case 'warehouse':
...
class _SalesCustomersContentState extends ConsumerState<_SalesCustomersContent> {
                ...
                children: [
                  CircleAvatar(
                    ...
                    child: Text(
                      ...
                      style: TextStyle(color: Colors.orange.shade700, fontWeight: FontWeight.bold, fontSize: 18),
                    ),
                  ),
          ...
          children: [
              ...
              child: Row(
                children: [
                  CircleAvatar(
                    ...
                    child: Text(
                      (customer['name'] ?? 'K')[0].toUpperCase(),
                      style: TextStyle(color: Colors.orange.shade700, fontWeight: FontWeight.bold, fontSize: 24),
                    ),
                  ),
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    _buildInfoRow(Icons.phone, 'Điện thoại', customer['phone'] ?? 'Chưa có'),
                    const SizedBox(height: 16),
                    _buildInfoRow(Icons.email, 'Email', customer['email'] ?? 'Chưa có'),
                    const SizedBox(height: 16),
                    _buildInfoRow(Icons.location_on, 'Địa chỉ', address ?? 'Chưa có địa chỉ'),
  ...
  Future<void> _openGoogleMaps(Map<String, dynamic> customer) async {
    // Build best VN address format: Số nhà + Tên đường, Phường, Quận, Thành phố
    final streetNumber = customer['street_number'] as String? ?? '';
    final street = customer['street'] as String? ?? '';
    final ward = customer['ward'] as String? ?? '';
    final district = customer['district'] as String? ?? '';
    final city = customer['city'] as String? ?? 'Hồ Chí Minh'; // Default to HCM
    final fallbackAddress = customer['address'] as String? ?? '';
    
    String formattedAddress;
    
    // Prioritize structured address parts for better accuracy
    if (street.isNotEmpty) {
      ...
      if (district.isNotEmpty) {
        // Add 'Quận' prefix if it's a number or doesn't have prefix
        if (RegExp(r'^\d+$').hasMatch(district)) {
          parts.add('Quận $district');
        } else if (!district.toLowerCase().startsWith('quận') && !district.toLowerCase().startsWith('huyện')) {
          parts.add('Quận $district');
        } else {
          parts.add(district);
        }
      ...
      // Fallback to address field if no structured data
      formattedAddress = fallbackAddress;
    }
    
    if (formattedAddress.isEmpty) return;
    
    // Add Vietnam for better international accuracy
    if (!formattedAddress.toLowerCase().contains('vietnam') && 
        !formattedAddress.toLowerCase().contains('việt nam')) {
      formattedAddress = '$formattedAddress, Việt Nam';
    }
    
    debugPrint('Opening Google Maps with address: $formattedAddress');
