...
class _ReportsPageState extends ConsumerState<ReportsPage>
  ...
  // Tabs that support date filtering (0=Doanh thu, 3=Đơn hàng)
  bool get _showDateFilter => _tabController.index == 0 || _tabController.index == 3;
      ...
      children: [
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  ...
                  Text(
                    _dateFilter != null ? getDateRangeLabel(_dateFilter!) : 'Tháng này (mặc định)',
                    style: TextStyle(
                      fontSize: 13,
                  ...
                  if (_dateFilter != null)
                    ...
                    Icon(Icons.arrow_drop_down, size: 22, color: Colors.grey.shade600),
                ],
              ),
            ),
          ),
        ...
        Expanded(
          child: TabBarView(
            controller: _tabController,
            children: [
              _RevenueReportTab(dateFilter: _dateFilter),
              const _ReceivablesReportTab(),
              const _InventoryReportTab(),
              _OrdersReportTab(dateFilter: _dateFilter),
            ],
          ),
        ),
      ],
...
class _RevenueReportTab extends ConsumerStatefulWidget {
  const _RevenueReportTab({this.dateFilter});
...
class _RevenueReportTabState extends ConsumerState<_RevenueReportTab> {
  ...
  @override
  void didUpdateWidget(covariant _RevenueReportTab oldWidget) {
  ...
  Future<void> _loadRevenueData() async {
    ...
    try {
      ...
      final startOfMonth = widget.dateFilter?.start ?? DateTime(now.year, now.month, 1);
      final endOfRange = widget.dateFilter != null 
          ? widget.dateFilter!.end.add(const Duration(days: 1)) 
          : now.add(const Duration(days: 1));
      final startOfWeek = widget.dateFilter?.start ?? now.subtract(Duration(days: now.weekday - 1));
      final startOfDay = widget.dateFilter?.start ?? DateTime(now.year, now.month, now.day);

      // Get today/range revenue
      final todayOrders = await supabase
          .from('sales_orders')
          .select('total, delivery_status, payment_status')
          .eq('company_id', companyId)
          .gte('created_at', startOfDay.toIso8601String())
          .lte('created_at', endOfRange.toIso8601String())
      ...
      // Get daily revenue for chart (last 7 days or within range)
      final last7Days = widget.dateFilter?.start ?? now.subtract(const Duration(days: 6));
      ...
      // Get top customers in range with proper customer names
...
class _OrdersReportTab extends ConsumerStatefulWidget {
  const _OrdersReportTab({this.dateFilter});
  final DateTimeRange? dateFilter;
...
class _OrdersReportTabState extends ConsumerState<_OrdersReportTab> {
  ...
  @override
  void didUpdateWidget(covariant _OrdersReportTab oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.dateFilter != widget.dateFilter) {
      _loadOrdersData();
    }
