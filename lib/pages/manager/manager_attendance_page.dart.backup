...
class _ManagerAttendancePageState extends ConsumerState<ManagerAttendancePage> {
  ...
  String? _branchName; // Store branch name for location
  ...
  void initState() {
    super.initState();
    // Schedule load after frame to ensure widget is mounted
  ...
  String _getErrorMessage(dynamic error) {
    ...
    if (errorStr.contains('already checked in') || errorStr.contains('đã chấm công')) {
      return 'Bạn đã chấm công vào hôm nay rồi';
    } else if (errorStr.contains('already checked out') || errorStr.contains('đã chấm công ra')) {
      return 'Bạn đã chấm công ra rồi';
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // TODO: Fetch actual branch name from database
      // For now, use 'Văn phòng' as default location name
      _branchName = 'Văn phòng';
  ...
  Future<void> _checkIn() async {
    ...
    try {
      ...
      try {
        ...
        if (!serviceEnabled) {
          gpsWarning = 'GPS không được bật';
          print('⚠️ [ManagerAttendance] Location services disabled');
        } else {
          // Check permissions
          ...
          if (permission == LocationPermission.denied || 
            ...
            gpsWarning = 'Quyền truy cập GPS bị từ chối';
            print('⚠️ [ManagerAttendance] Location permission denied');
          } else {
            position = await Geolocator.getCurrentPosition(
              ...
              onTimeout: () {
                print('⚠️ [ManagerAttendance] GPS timeout');
        ...
        gpsWarning = gpsWarning ?? 'Không thể lấy vị trí GPS';
        print('⚠️ [ManagerAttendance] GPS error: $e');
      }

      // Show warning if GPS unavailable
      if (mounted && gpsWarning != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('⚠️ $gpsWarning - Tiếp tục chấm công không có vị trí'),
      ...
      await _attendanceService.checkIn(
        ...
        location: _branchName ?? 'Văn phòng',
      );
  ...
  Future<void> _checkOut() async {
    ...
    try {
      ...
      print('🌍 [ManagerAttendance] Getting GPS location for check-out...');
      ...
      try {
        ...
        if (!serviceEnabled) {
          ...
          if (permission == LocationPermission.denied || 
            ...
            position = await Geolocator.getCurrentPosition(
              ...
              const Duration(seconds: 10),
              onTimeout: () {
                print('⚠️ [ManagerAttendance] GPS timeout on check-out');
                throw TimeoutException('GPS timeout');
              },
            );
            print('✅ [ManagerAttendance] GPS: ${position.latitude}, ${position.longitude}');
        ...
        // GPS not available, continue without it
        print('⚠️ [ManagerAttendance] GPS error on check-out: $e');
      }
