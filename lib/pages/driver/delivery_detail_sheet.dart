...
class DeliveryDetailSheet extends StatelessWidget {
  ...
  void _showQRTransferDialog(BuildContext context, double amount, String orderNumber) async {
    try {
      ...
      final empData = await supabase.from('employees').select('company_id').eq('auth_user_id', user.id).maybeSingle();
      ...
      final companyData = await supabase.from('companies').select('bank_name, bank_account_number, bank_account_name, bank_bin').eq('id', empData['company_id']).maybeSingle();
